...
import umc.study.service.MissionService.MyMissionQueryService;
...
public class MemberRestController {
    ...
    @Operation(summary = "특정 회원의 도전중인 목록 조회 API", description = "특정 회원의 도전중인 미션의 목록을 조회하는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200", description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH004", description = "acess 토큰 만료", content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    public ApiResponse<MyMissionResponseDTO.MyMissionPreviewListDTO> getMyMissionList(@ExistMember @PathVariable(name = "memberId") Long memberId, @CheckPage @RequestParam(name = "page") Integer page) {
        Page<MemberMission> missionList = myMissionQueryService.getMissionList(memberId, page);
        return ApiResponse.onSuccess(MemberMissionConverter.myMissionPreViewListDTO(missionList));
    }
}
